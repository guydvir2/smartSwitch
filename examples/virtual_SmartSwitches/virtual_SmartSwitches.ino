             ...
             int timeout = 0, const char *name = "SW")
{
  ...
  {
    sw_array[total_sw_counter]->set_output();
  }
  if (use_indic)
  {
...
void postMQTT_virtualCMDs(uint8_t i,  char *topic, uint8_t stat)
{
  const char *state[] = {"off", "on"};
  iot.pub_noTopic(state[stat],topic);
}
void setup()
{
  Serial.begin(115200);
  ...
  init_sw(true /*use_input*/, ON_OFF_SW /*input_type*/,
          false /*use_output*/, 0 /*pwm_intense*/, HIGH /* output_on_dir*/,
          true /*use_indic*/, HIGH /*indic_dir*/,
          0 /*timeout_sec*/, "myHome/Lights/bed" /*Switch_name MQTT topic*/);
...
void loop()
{
  ...
  for (uint8_t i = 0; i < total_sw_counter; i++)
  {
    if (sw_array[i]->loop())
    {
      postMQTT_virtualCMDs(i,sw_array[i]->name,sw_array[i]->telemtryMSG.state);
